What kind of operation does a MUX implement?

---

Selection — a ternary operator for two inputs.

![3-to-1 MUX as AND/OR tree on select signals](mux.png)
...
What would a 4-bit ripple-carry adder made of 1-bit full adders look like?
...
![4-bit ripple-carry adder](ripple-carry-adder.png)

%%

Walk through the calculation of the area taken up by an int8 × int8 multiply-accumulate. Relevant gate counts: full adder = 1.0, 2:1 mux = 0.3.
...
Visualize how you can use a fused multiply-add to compute the dot product of two vectors.
...
![Chained FMAs computing a dot product](fma-dot-product.png)

%%

Walk through the 5 steps of the RTL-to-GDS PD flow.
...
Why can two chips on the same TSMC node run at 4 GHz vs 5 GHz?
...
What is the tradeoff between registers and SRAM?
...
Why can you only read OR write one column of an SRAM array per cycle?
...
![SRAM array layout: one decoder, shared word/bit lines](sram-array.png)
