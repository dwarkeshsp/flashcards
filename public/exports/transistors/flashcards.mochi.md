...
Does a diode act as a conductor or insulator when conventional current flows from $\text{battery} \to \text{P} \to \text{N} \to \text{battery}$? Why?
...
Does a diode act as a conductor or insulator when conventional current flows from $\text{battery} \to \text{N} \to \text{P} \to \text{battery}$? Why?
...
In a BJT transistor, if a small positive current is allowed to flow from base to emitter, what is allowed to happen?

---

A large positive current is allowed to flow from collector to emitter.

![BJT control flow](bjt-current-flow.png)

*Image from Ben Eater's transistor video.*

%%

If electrons are flowing from emitter to base (i.e. you turn the transistor on), why does that enable those electrons to now reach the collector?
...
![NPN BJT electron flow: emitter → base → collector](bjt-electron-flow.png)

*Image from Ben Eater's transistor video.*

%%

Sketch in your head what a MOSFET looks like.
...
![NMOS cross-section](mosfet-cross-section.png)

%%

Why does a positive charge at the gate open up the channel underneath the insulator? (Consider an NMOS.)
...
Why is a MOSFET more power efficient than a bipolar junction transistor?
...
Conceptually, why doesn't it require a continuous current to keep the MOSFET channel between source and drain open?
...
What is the only time you have to pay an energy cost in a MOSFET?
...
In a MOSFET, you can think of the positive gate / SiO₂ insulator / negative channel triplet as what kind of component?
...
How did MOSFETs enable Dennard scaling?
...
The original 1947 transistor was made of germanium, which also has 4 valence electrons like silicon. Why can't germanium be used to build MOSFETs?
...
At a high level, what is the difference between planar, FinFET, and gate-all-around?
...
MOSFET technology has progressed from planar (1965–2011) to FinFET (2011–2022) to gate-all-around (2022 onwards). Why?
