...
Equation for $t_{\text{compute}}$ — not accounting for attention
...
Equation for $t_{\text{mem}}$ (hint — there's a contribution from the weights, and from the KV cache)
...
Sketch out (in your head) what the graph with batch size on the x-axis and latency on the y-axis looks like — draw the lines for $t_{\text{compute}}$, KV fetch, and weight fetch, then bold the line that corresponds to total latency given a certain batch size.

---

![Latency vs. batch size](latency-vs-batch.png)
...
Work through the math that shows that optimal batch size ought to be at least $300 \times$ your sparsity ratio (active / total parameters) to maximize throughput. Ignore KV cache.
...
Picture a GPU cluster as a train station: every $\sim$20ms, a "train" departs carrying a batch of sequences through one forward pass (producing one new token per sequence). Why 20ms specifically? What goes wrong if you schedule trains more frequently? How about less frequently?
...
![Pipeline bubbles diagram](pipeline-bubbles.png)
...
Why might you naively expect $C_{\text{pretrain}} = C_{\text{RL}} = C_{\text{inference}}$?
...
Solve for $D_{\text{pretrain}} = D_{\text{RL}} = D_{\text{inference}}$, with $\tfrac{1}{3}$ as much MFU from decode as prefill.
...
The Chinchilla rule is that $D_{\text{optimal}} \approx 20 \times N_{\text{active}}$. If a frontier model has 100B active parameters and is pretrained on 200T tokens, how much over Chinchilla-optimal is it?
...
![Cost vs. context length](cost-vs-context.png)
...
Why are cached input tokens (cache hits) $\sim 10\times$ cheaper than fresh input tokens?
