Why is breaking causality in training so bad?
...
What is expert choice, and why does it break causality?
...
What is token dropping, and why does it break causality?
...
Why is bias much worse than variance in training?
...
Name the rumored reasons why the Llama 4, Gemini 2 Pro, and GPT-4 training runs got fucked up, respectively.
...
What happens in a reduce-scatter?
...
![All-reduce vs reduce-scatter vs all-gather](collectives.png)

%%

Time cost of an all-gather within one domain of $N$ GPUs, where $B$ is the total payload and $W$ is per-GPU egress bandwidth?
...
Time cost of an all-to-all within one domain of $N$ GPUs is
...
What is the problem with data parallelism alone?
...
How does fully sharded data parallelism (FSDP) work?
...
Why is it much more trivial to overlap compute and comms for FSDP than it is for, say, expert or tensor parallelism?
...
Why does FSDP only have a 50% comms volume overhead over vanilla DP? You'd think having to all-gather every layer's full weights, use them for one matmul, then throw them away, would be hugely taxing.
...
Why can MFU totally crater as you scale the number of GPUs on FSDP? Why did Dylan joke that FSDP is for the GPU poor?
...
Define the critical batch size.
...
Why does FSDP set a batch size floor?
