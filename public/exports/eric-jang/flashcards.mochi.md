When AlphaZero is going to make a move, it kicks off MCTS with an empty tree. For each simulation of MCTS (1600 per move), what happens?
...
![One MCTS simulation](mcts-three-steps-paper.png)

*Figure 2 from [Silver et al., 2017](https://discovery.ucl.ac.uk/id/eprint/10045895/1/agz_unformatted_nature.pdf).*

%%

As you keep revisiting a node in MCTS, you choose the child node to explore based on which one has the highest PUCT score, which is calculated as:
...
![PUCT explore vs exploit dominance shift](puct-dominance-shift.png)

%%

Of the four search-time quantities in PUCT — $Q(s,a)$, $P(s,a)$, $N(s)$, $N(s,a)$ — which is produced by the neural network and which live in the MCTS tree node?
...
When a simulation reaches a newly evaluated leaf and produces a value, every ancestor node on the path back to the root updates its stored $Q$. What statistic does $Q(s,a)$ end up representing?
...
What is the overall purpose of the AlphaGo neural network in the full program?
...
The AlphaGo network takes in ___ and outputs both ___ and ___.
...
![AlphaGo network as MCTS guide](network-schematic.png)

%%

If there are up to ~361 legal moves per turn and a Go game can last ~300 turns, the naive game tree has on the order of $361^{300}$ trajectories. Which axis does the policy head prune, and which does the value head prune?
...
![Two heads, two cuts](breadth-depth-tree.png)

%%

Couldn't we drop the policy head and pick $a^* = \arg\max_a V_\theta(s')$ over the resulting next states $s'$? Why is that a bad idea? Two reasons:
...
AlphaGo, AlphaZero, and KataGo all use convolutional ResNets rather than Transformers. Eric tried Transformers for Go at his scale and couldn't beat ResNets. Why do CNN inductive biases fit Go better?
...
The AlphaGo network doesn't take into consideration previous board states. Why can it get away with that, and when would that not be possible?
...
We update the AlphaGo network with per-move training labels. What are these labels?
...
The AlphaZero loss is composed of two quantities. What are they conceptually, and mathematically?
...
Eric ran a self-play loop that used MCTS for action selection but trained the policy net only on the moves from games it *won* (REINFORCE-style winner-imitation). It plateaued at ~50% against KataGo. Why?
...
Both MCTS (AlphaZero) and NFSP (AlphaStar) relabel each visited state $s$ with a better action $a^*$ for the student policy to imitate. They differ only in where $a^*$ comes from. What is the duality?
...
![MCTS and NFSP — same student, opposite time-directions](mcts-nfsp-time-direction.png)

%%

In the DeepSeek-R1 paper they said they weren't able to get MCTS to work for LLMs. What were the two big issues?
...
![Why MCTS doesn't transfer from Go to LLMs](mcts-go-vs-llm.png)
