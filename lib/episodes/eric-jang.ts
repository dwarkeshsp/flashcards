// Cards for Eric Jang's blackboard lecture on AlphaGo. Hand-curated:
// the source of truth is content/eric-jang-cards.md. Re-running
// scripts/pipeline/run.ts on this slug will overwrite this file.
  ...
  sections: [
    {
      ...
      cards: [
        ...
        {
          q: `The AlphaGo network takes in \\_\\_\\_ and outputs both \\_\\_\\_ and \\_\\_\\_.`,
...
- MCTS node: $Q(s,a)$, $N(s)$, $N(s,a)$ — all running statistics of the search itself.`,
        },
        {
          ...
          a: `$$Q(s,a) \\;\\leftarrow\\; \\frac{N(s,a)\\,Q(s,a) + v}{N(s,a) + 1}, \\qquad N(s,a) \\leftarrow N(s,a) + 1.$$
...
MCTS distillation has no credit-assignment problem. Instead of "this game was won, copy these moves," it says: *at every state you visited, here is a strictly better move than the one you played.* Every move becomes a dense per-state supervision target — like DAgger interventions in imitation learning.`,
        },
        {
          ...
          a: `- NFSP — search backward in time. Bellman/TD backup over trajectories that *already happened*. Teacher = greedy $a$ from learned $Q$.
