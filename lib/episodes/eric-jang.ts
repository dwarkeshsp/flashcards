...
export const ericJang: Episode = {
  ...
  guest: "Eric Jang",
  blurb: "Practice questions for Eric Jang's blackboard lecture on building AlphaGo from scratch: Go, MCTS, policy and value networks, and how AlphaZero-style learning compares to LLM RL.",
  date: "2026-05-08",
  transcriptPath: "transcripts/eric-jang.md",
  note: "Generated end-to-end by parallel Cursor SDK agents (Opus 4.7) with web search and matplotlib visual tooling. See scripts/pipeline/run.ts.",
  sections: [
    {
      id: "why-alphago-s-rl-is-elegant",
      title: "Why AlphaGo's RL is elegant",
      timestamp: "01:02:31",
      cards: [
        {
          q: `Why does AlphaGo's training avoid the sparse-reward exploration problem that LLM RL has to solve?`,
          a: `Because **MCTS run on top of the current policy net $\\pi_\\theta$ is a policy-improvement operator**: its visit-count distribution is (typically) a strictly better policy than $\\pi_\\theta$ itself. So $\\pi_\\theta$ is just trained to imitate the MCTS distribution — supervised learning on *improved labels*, with no need to first achieve a non-zero success rate by random exploration.
...
![Why AlphaGo's RL is elegant](/images/eric-jang/alphago-bypasses-sparse-rl.png)`,
    ...
    {
      id: "self-play-as-policy-improvement",
      title: "Self-play as policy improvement",
      timestamp: "01:07",
      cards: [
        {
          q: `In AlphaGo's self-play loop, every position's MCTS visit distribution is used as a supervised label for the policy net. On a test-time-scaling plot (x = num MCTS simulations, y = win rate), what does this distillation step do to the curve from iteration $k$ to iteration $k{+}1$, and why?`,
          a: `Iter $k{+}1$'s curve **starts where iter $k$'s curve ended after $N$ simulations.** Distilling the visit distribution amortizes those $N$ sims of search into the policy weights, so the new "shoot-from-the-hip" net plays at iter $k$'s post-search level for free.
...
![Why MCTS hasn't taken over LLMs](/images/eric-jang/mcts-fails-on-llms.png)`,
    ...
    },
    {
      ...
      cards: [
        {
          q: `In AlphaGo's MCTS, name the four phases of a single simulation, **in order**.`,
          a: `**select → expand → evaluate → backup**
...
![Why naive self-play stalls](/images/eric-jang/naive-winner-imitation-fails.png)`,
    ...
    {
      id: "action-selection-inside-mcts",
      title: "Action selection inside MCTS",
      timestamp: "00:29:48",
      cards: [
        {
          q: `Write AlphaGo's **PUCT** rule for picking which child to descend into during MCTS, and label what each of its four pieces — $Q(s,a)$, $c_{\\mathrm{PUCT}}$, $P(a)$, and $\\sqrt{N}/(1+N_a)$ — contributes.`,
          a: `$$a^{\\star} \\;=\\; \\arg\\max_{a}\\;\\Bigl[\\,Q(s,a)\\;+\\;c_{\\mathrm{PUCT}}\\,P(a)\\,\\frac{\\sqrt{N}}{1+N_a}\\,\\Bigr]$$
...
![Action selection inside MCTS](/images/eric-jang/puct-selection-rule.png)`,
    ...
    {
      id: "rl-vs-supervised-learning-information-rate",
      title: "RL vs supervised learning information rate",
      timestamp: "03:28:00",
      cards: [
        {
          q: `If your policy's pass rate on a token (or trajectory) is $p$, how many bits per sample does cross-entropy supervised learning extract, and how many does naive binary-reward RL extract? What does each do as $p \\to 0$?`,
          a: `- **SL (cross-entropy on the labeled token):** $-\\log p$ bits per sample, deterministic given the label. Diverges to $\\infty$ as $p \\to 0$ — every miss tells you exactly how far the predicted distribution sits from the label.
...
![RL vs supervised learning information rate](/images/eric-jang/supervised-vs-rl-bits-per-sample.png)`,
    ...
    {
      id: "computer-go-scoring",
      title: "Computer Go scoring",
      timestamp: "00:13:59",
      cards: [
        {
          q: `Why do Go AIs train and resolve games under **Tromp-Taylor** rules instead of standard human (e.g. Chinese) rules?`,
          a: `Tromp-Taylor scoring is **fully algorithmic** — it removes the human "agree this group is dead" consensus step.
...
![Go fundamentals](/images/eric-jang/two-eyes-unconditionally-alive.png)`,
    ...
    {
      id: "why-neural-nets-help",
      title: "Why neural nets help",
      timestamp: "00:39:03",
      cards: [
        {
          q: `In AlphaGo's MCTS, what does the value head $V_\\theta(s)$ compute as a "shortcut for searching to the end of the tree"?`,
          a: `$V_\\theta(s) \\approx \\mathbb{E}[U \\mid s]$, the expected terminal outcome of self-play continued from $s$ — one forward pass replaces an exponential-depth rollout the network never explicitly enumerates.
