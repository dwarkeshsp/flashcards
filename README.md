...
Static site with two sections:

- **Reads** (`/reads/<slug>/`): notes and Kindle highlights on books
  Dwarkesh is reading (articles may join later).
- **Flashcards** (`/<slug>/`): practice flashcards for technical
  Dwarkesh Podcast episodes, generated by a Cursor SDK agent pipeline.
...
Each read is a markdown file at `content/reads/<slug>.md`:

```markdown
---
title: Augustus
author: John Williams
year: 1972            # publication year
kind: book            # or "article"
status: reading       # or "finished"
date: 2026-07         # when finished (optional)
sourceUrl: https://…  # for articles (optional)
blurb: One-liner for index lists (optional)
readwise: "Exact Readwise title"  # only if title matching fails
---

The note itself: a few paragraphs of markdown about the book.
```

`lib/reads.ts` loads these at build time; the home page and `/reads/`
list them, and each gets a page at `/reads/<slug>/`.

### Kindle highlights via Readwise

Highlights live next to the note in
`content/reads/<slug>.highlights.json`. That file is **machine-owned**:
it's written by the importer and overwritten on every re-import, so
never edit it by hand — your own prose belongs in the `.md` file.
On the read page, a highlight's attached Kindle note renders as a
Tufte-style margin note.

Setup (once): create a [Readwise](https://readwise.io) account, connect
Kindle sync, and grab a token from
[readwise.io/access_token](https://readwise.io/access_token). Then:

```bash
READWISE_TOKEN=xxx npm run import-readwise
```

The importer fetches your full Readwise library, matches books to read
stubs by title (or by the `readwise:` frontmatter key), writes the
matched highlights, and lists any Readwise books that have no stub yet.
Re-running is idempotent — new highlights simply appear on the next
import. You can also import from a saved export dump with
`npm run import-readwise -- --file export.json`.

## Flashcards

Cards are generated end-to-end by a Cursor SDK agent pipeline that fans
out one Opus 4.7 worker per card, with web search and matplotlib visual
tooling. Each episode lives at `/<slug>/` with its own Markdown,
...
| Path | What it is |
| --- | --- |
| `content/reads/` | Read notes (`.md`) and imported highlights (`.highlights.json`) |
| `lib/reads.ts` | Build-time loader for reads |
| `app/reads/` | Reads index and per-read pages |
| `scripts/import-readwise.ts` | Readwise highlight importer |
