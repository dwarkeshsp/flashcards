---
slug: pretraining
title: Pretraining parallelisms and failed runs
kind: subject
blurb: Why pretraining runs fail, and the chain of problems and fixes behind FSDP / pipeline / tensor parallelism.
substack: https://www.dwarkesh.com/p/notes-on-pretraining-parallelisms
---

# Editing notes

- Source: Dwarkesh's notes on pretraining parallelisms (linked above),
  which combine a conversation about failed training runs with notes
  from Horace He's lecture.
- One card per `## Q:` / `### A:` pair. Card boundaries are `---`.
- LaTeX is `$...$` inline and `$$...$$` block.

---

# Section: Why pretraining runs fail
id: why-runs-fail

## Q: Why is breaking causality in training so bad?
...
## Q: What is expert choice, and why does it break causality?
...
## Q: What is token dropping, and why does it break causality?
...
## Q: Why is bias much worse than variance in training?

### A: Variance can average out, but bias compounds.

---

## Q: Name the rumored reasons why the Llama 4, Gemini 2 Pro, and GPT-4 training runs got fucked up, respectively.
...
## Q: What happens in an all-gather?

### A: Every GPU starts with a different shard. Afterwards, every GPU has the full concatenated result.

---

## Q: What happens in an all-reduce?

### A: Each GPU ends up with the element-wise reduction (i.e. sum) across every GPU.

---

## Q: What happens in a reduce-scatter?
...
## Q: Time cost of an all-gather within one domain of $N$ GPUs, where $B$ is the total payload and $W$ is per-GPU egress bandwidth?

### A: $$T_{\text{AllGather}} = \frac{B \cdot (N - 1)}{W \cdot N}$$

---

## Q: Time cost of an all-to-all within one domain of $N$ GPUs is

$$T_{\text{AllToAll}} = \frac{B \cdot (N - 1)}{W \cdot N^2} \approx \frac{B}{W \cdot N}.$$

Explain why.

### A: Each GPU starts with $B / N$ of info. It needs to send $1/N$ of that to each of the $N - 1$ other GPUs.
...
## Q: What is the problem with data parallelism alone?

### A: Each GPU only has a limited amount of HBM — a B300 is 288 GB — and this is not enough to store the weights as models get bigger, much less their activations.

---

## Q: How does fully sharded data parallelism (FSDP) work?
...
## Q: Why is it much more trivial to overlap compute and comms for FSDP than it is for, say, expert or tensor parallelism?
...
## Q: Why does FSDP only have a 50% comms volume overhead over vanilla DP? You'd think having to all-gather every layer's full weights, use them for one matmul, then throw them away, would be hugely taxing.
...
## Q: Why can MFU totally crater as you scale the number of GPUs on FSDP? Why did Dylan joke that FSDP is for the GPU poor?

### A: Compute time decreases as you add more GPUs, but comms time does not.

---

## Q: Define the critical batch size.

### A: The batch size beyond which doubling the batch stops halving the number of steps to reach a target loss.

---

## Q: Why does FSDP set a batch size floor?

### A: FSDP is data-parallel, so each GPU processes at least one sequence. Attention is computed within a sequence and can't (easily) be split across GPUs.
